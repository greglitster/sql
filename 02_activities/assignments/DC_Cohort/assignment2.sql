...
SELECT 
    product_name
    || ', ' || COALESCE(NULLIF(product_size, ''), '')
    || ' (' || COALESCE(NULLIF(product_qty_type, ''), 'unit') || ')'
FROM product;
...
WHERE x.market_date < '2022-04-29';
...
FROM
	(SELECT
		...
		cost_to_customer_per_qty,
		transaction_time,
		COUNT() OVER(PARTITION BY product_id, customer_id) AS product_count
...
(SELECT DISTINCT
	...
	p.product_name, 
	v.vendor_name, 
	(vi.original_price*5) AS product_sale
...
SELECT vendor_name, product_name, SUM(product_sale)
FROM product_sales CROSS JOIN customer_list
...
VALUES(24, 'Apple Fritter', '1 dozen', 3, 'unit', CURRENT_TIMESTAMP)
