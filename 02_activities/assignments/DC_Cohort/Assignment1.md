...
```
Perhaps the sign of a well-designed data-base or data system is that you don't notice when you are interacting with it. Generally, this is how I feel when I am considering the values that are embedded into the systems - not noticeable without explicit effort to interrogate their design. Learning about how a system works is a valuable pursuit, but the need or desire to do that doesn't seem to come around until you personally come up against a puzzling case, like the situation faced by Riz as described in the article. In this case, significant limitations were placed on Riz, introducing barriers to access social services, banking, and health. As highlighted in the article, seemingly menial choices made by system architects can have significant adverse effects years later that go far beyond the technical aspects of those systems. 

I know that there are value systems including in the technology that I interact with, but I find I don't put much thought into what those values are until I seemingly have to work against the system to achieve my desired outcome. It is in this case that I will want to enact some kind of change in the system to better serve the needs of the people that interact with it. My main curiosity in this respect is deciding on an appropriate threshold for when we need to enact that change. How many 'similar to' Riz's stories does the government or funding body need to hear in order to start using resources to solve a problem?  Conversations where those thresholds are decided, much like decisions about the systems architecture itself, will be brimming with human values that inevitably require thoughtful consideration for how they are defined and utilized in those spaces. But untill those thresholds are decided, it isn't clear when solution generation to those problems will begin. Those solutions should not take a patch-work style approach either, potential requiring complete overhauls. This would not be a straightforward endeavour but one that is worth spending time on in order to improve the tools that are at our disposal. 
```
